...
function tvRP.giveWeapons(weapons, clear_before, save)
    ...
    if save == nil or save == true then
        ...
        -- vRPold: vRPserver.updateWeapons({tvRP.getWeapons()})
    end
end

function tvRP.giveWeapons2(weapons,clear_before)
	local player = PlayerPedId()
	if clear_before then
		RemoveAllPedWeapons(player,true)
	end
...
function tvRP.setArmour2(amount)
...
function tvRP.setArmour(amount, save)
    ...
    if save == nil or save == true then
        ...
        -- vRPold: vRPserver.updateArmor({tvRP.getArmour()})
    end
end
