...
-- QUERYES
-----------------------------------------------------------------------------------------------------------------------------------------
DB.prepare("groups/createBlacklistTable",[[
  CREATE TABLE IF NOT EXISTS `groups_blacklist` (
    `user_id` INT(20) UNSIGNED NOT NULL,
    `time` INT(100) UNSIGNED NOT NULL DEFAULT '0'
  )
]])
...
Citizen.CreateThread(function ()
	DB.execute('groups/createBlacklistTable')
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
				...
				if ok then
					...
					local datawebhook = {
						{ 
							...
							footer = { 
								text = "Registro em: "..os.date("%d/%m/%Y | %H:%M:%S"), 
					...
					sendwebhook(Webhooks.painelContratacao, datawebhook)
